...
`_ = numpy.arange(min_spin, max_spin, step)`<br/>
where step is given by <br/>
`step = (max_spin - min_spin) / Nbh`.<br/>
_ is a numpy array of lenght Nbh. Spin values are extracted from _ using an array of probabilities p generated by<br/>
`gaussian = rnd.normal(spin_mean, self. spin_sigma, int(1e6)) `<br/>
`h, bin_edges = np.histogram(gaussian, bins = self.Nbh, density = True)`<br/>
`p = h * np.diff(bin_edges)`<br/>
than a simple random choice is used<br/>
`spins = numpy.random.choice(_, size = Nbh, p = p)`<br/>
